...
//echo "reseller id".$reseller_id;die();
//remark and date
...
if($sign){
    ...
    $signature = $sign_url."uploads/default-signature.jpg";
}
...
// die;
// pre( $apt_value , 1 );
suitable_career_identification($pdf , $code, $con , $p1_2_per , $apt_value);
...
// Interest Mapper
    ...
    function suitable_career_identification($pdf , $code , $con , $usr_int , $usr_apt , $car_count = 20){
        //i.	Get unique careers from career_apt table.
        // pre( [  $usr_int , $usr_apt  ] , 1 );
        ...
        foreach( $arr_apt as $k => $v ){
            ...
            // pre( [$scr_car , $lvl_apt , $scr_apt , $apt_high , $lvl_apt_SA , $lvl_int ] );
            $car_reco = '';
        ...
        foreach( $arr_car_reco as $key => $value ){
            ...
            // elseif( $value['car_reco'] == 'Explore' ){
            //     $arr_choices['Explore'][] = $value;
            // }
        }
        ...
        // pre( $arr_car_to_print , 1 );
        $arr_edu_map = get_edu( $con , $arr_car_to_print , $arr_tbl['edu'] );
        // pre($arr_edu_map , 1);
        $pagecount = $pdf->setSourceFile('report_template/uce-streams-clusters.pdf');
        // pre( $arr_edu_map);
        // pre( $arr_car_to_print ,  1);
        print_stream( $pdf , $arr_stream_map );
        print_cluster( $pdf , $arr_cluster_map );
        // pre( $arr_car_to_print , 1 );
        print_careers($con , $pdf , $arr_car_to_print , $arr_edu_map , $code);
        
    }
    
    // function chk_mandantory_apt( $apt_stu , $car_man_apt ){
    //     foreach( $car_man_apt as $v ){
            
    //     }
    // }
    
    function get_edu( $con , $arr_car , $tbl_name ){
        foreach( $arr_car as $key => $car ){
            ...
            $arr_edu[$key] = mysqli_fetch_array(  $result );
            $arr_edu[$key]['scr_car'] = $scr_car;
        }
        return $arr_edu;
        // $careers = implode( "','" , $arr );
        // $query = $sql."SELECT profession_name, stream_no, 11th_12th, Education, indian_name  FROM  ".$tbl_name." WHERE profession_name IN ('".$careers."')";
        // $arr[] = $car['profession_name'];
        // pre( $sql , 1 );
        // $result = mysqli_query( $con , $sql );
        // while( $data = mysqli_fetch_array(  $result ) ){
        //     $arr_edu[] = $data;
    ...
    function print_stream($pdf , $arr_stream_map){
        ...
        foreach( $t as $k => $v){
            ...
            // pre( $y." ".$k );
            $pdf->SetXY($x1,$y);
            $pdf->Cell(0,20,$tg, 0, 1,'C',false);
            $pdf->SetXY( $x2, $y );
            $pdf->Cell(0,20,$o[$k], 0, 1,'C',false);
            // $pdf
            $i++;
        }
        // die;
    ...
    function print_careers($con , $pdf , $arr_to_print , $arr_edu_map , $code){
        // pre( $arr_to_print );
        // pre( $arr_edu_map , 1 );
        $pdf->addPage();
        // $pdf->useTemplate($tpl, ['adjustPageSize'=>true]);
        ...
        // pre( $arr_edu_map , 1 );
        foreach($arr_edu_map as $k=>$v)
        {
            // pre( $v );
            //Lines added on 25-04-22 - to adapt for SA   
            $cluster = $v['Cluster'];
            // $reco = $v[1];
            if( $arr_to_print[$k]['common_name'] == null ){
                $prof = $v['profession_name'];
            }
            else{
                $prof = $arr_to_print[$k]['common_name'];
            }
            $stream = $v['11th_12th'];
            $edu = $v['Education'];
            if( $scr >= 100 ){
            ...
            echo __LINE__."<br>" ;
                ...
                for ($i = 0; $i < $cells; $i++) 
                {
                    ...
                    else 
                    {
                        ...
                        $pdf->SetXY($x + (150) , $y);
                        $pdf->MultiCell(20, $height, $pri_scr ,0,'L');
                    }
...
ob_end_clean();
$pdf->AliasNbPages();
$filename = $code."-".str_replace(' ', '-', $detail_row['name'] ).".pdf";
