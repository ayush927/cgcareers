...
class Core_model extends CI_Model{
  ...
  function update_batch_by_where($table , $data , $where){
  ...
  function joinQuery($options){
    ...
    if( $subquery ){
        $this->db->where('`'.$subquery['col'].'` '.$subquery['con'].$subquery['query'], NULL, FALSE);
    }
    ...
    if ($where_not_in_numeric != false) {
      foreach ($where_not_in_numeric as $key => $value) {
      ...
      if(is_array($like)){
        foreach ($like as $index => $value){
          if( count( $value ) == 3 ){
            ...
            if($index == 0){
              $this->db->like($value[0], $value[1]);
            }
            else{
              $this->db->or_like($value[0], $value[1]);
            }
          }
        }
      }
