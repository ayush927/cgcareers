<?php 
    class Career_library extends CI_Controller
    {
        ...
        public function image_thumbnail( $imageName = null ){
            if( $imageName == null ){
                $imageList = getQuery([ 'table' => 'profession_images' ]);
                if( !empty( $imageList ) ){
                    foreach ($imageList as $key => $value) {
                        if( $value['thumbnail_name'] == '' ){
                            make_thumb( 'https://users.respicite.com/uploads/galleryImage/'.$value['imageName'] , 'https://users.respicite.com/uploads/thumbnail/' , 350 , 260  );
                        }
                    }
                }
            }
        ...
        public function add_profession($id = null){
            ...
            if( $id != null  ){
                ...
                if( !empty( $data['edit'] ) ){
                    $data['checkProfession'] = getQuery( [ 'table' => 'c_library_1' ,'where' => [ 'profession_name' => $data['edit']['standard_title'] ] ,'single' => true ] );
                    $data['checkProfessionClib'] = getQuery( [ 'table' => 'c_library_1' ,'where' => [ 'c_lib_name' => $data['edit']['standard_title'] ] ,'single' => true ] );
                    $data['list2'] = getQuery( [ 'table' => 'c_library' , 'select' => 'Path' , 'where' =>  [ 'Cluster' => $data['edit']['Cluster'] ] ,  'order' => [ 'Path' => 'ASC' ] , 'group' => [ 'Path' ] ] );
                }
                else{
                    $this->session->set_flashdata( 'msg' , 'Parameter are wrongs');
                    $this->session->set_flashdata('status' , 'info');
                    redirect( 'adminController/view-career-cluster-details' , true );
                }
            }
            $data['professionList'] = getQuery( [ 'table' => 'c_library_1' , 'select' => 'SNo , profession_name' , 'order' => [ 'profession_name' => 'ASC' ]  ] );
            // pre($data , 1);
            ...
            if(!empty($_POST)){
                ...
                if($this->form_validation->run() == false){
                    ...
                    if( isset( $_POST['id'] ) ){
                        ...
                        $checkVar = 0;
                        if( isset( $c_lib_1_profession_id  ) ){
                            if( $c_lib_1_profession_id != 'Select Profession Name' ){
                                $checkVar = 1;
                                $c_library['cdb_status'] = $cdb_status;
                            }
                        }
                        if( isset( $imageId ) ){
                        ...
                        if( $checkVar == 1 ){
                            ...
                            $c_library1_data = [
                                'c_lib_name' => $standard_title,
                            ];
                            // pre( $c_lib_1_profession_id );
                            // pre( $c_library1_data  , 1);
                            update([ 'table' =>  'c_library_1' , 'data' =>  [ 'c_lib_name' => NULL ], 'where' => [ 'c_lib_name' => $standard_title ] ]);
                            $status = update([ 'table' =>  'c_library_1' , 'data' =>  $c_library1_data, 'where' => [ 'SNo' => $c_lib_1_profession_id ] ]);
                        }
                        else{
                            $c_library1 = [
                                'profession_id' => $profession_id,
                            ...
                            // pre( $c_library1 , 1 );
                            $checkDetail = getQuery( [ 'where' => [ 'profession_id' => $_POST['profession_id'] ] , 'single' => true ,  'table' =>  'c_library_1'  ] );
                            if( !empty( $checkDetail  )){
                                $status = update([ 'table' =>  'c_library_1' , 'data' =>  $c_library1, 'where' => [ 'profession_id' => $_POST['profession_id'] ] ]);
                            }
                            else{
                                $id = insert( 'c_library_1' , $c_library1 );
                            }
                            $status = update([ 'table' =>  'c_library' , 'data' =>  $c_library , 'where' => [ 'profession_id' => $_POST['profession_id'] ] ]);
                        }
                        // lQ(1);
                        if( affected() > 0 ){
                            $this->session->set_flashdata( 'msg' , 'Updated Career Cluster');
