    ...
    class BaseController extends CI_Controller
    {
        ...
        {   $code=$this->session->user['code'];
            ...
            if( $user['iam'] == "user" ){
                ...
                redirect(base_url().'/UserController/login/'.base64_encode($code));
            }
        }
        // end logout function
        ...
        {   
            ...
            if(isset($_POST['regbtn']))
            {
                ...
                if($this->form_validation->run() == true)
                {
                    ...
                    if( isset($code) ){
                        if( $send ){
                            redirect('/BaseController/validate_otp/'.$code);
                        }
                    }
                    else{
                        if( $send ){
        ...
        public function initializer()
        {
            ...
            $data['reseller_sp'] = getQuery( [ 'where' => [ 'user_id' => $data['user']['user_id'] ] , 'table' => 'user_details' , 'single' => true  ] );
         ...
         public function counselingParameters()
        {
            ...
            if( in_array("seller_config", explode(',',$data['allowed_services'])) ){
                $calendly_data = getQuery( ['table' => 'calendly_event' , 'where' => [ 'user_id' => $user['id'] ]]);
                if( !empty($calendly_data) ){
                    if(count($calendly_data) > 0 && count( $calendly_data ) < 3 ){
                        $calendly_data = getQuery( ['table' => 'calendly_event' , 'where' => [ 'user_id' => $user['id'] , 'status' => 'active' ] ] );
                        $admin_calendly_data = getQuery( [ 'table' => 'admin_calendly_event' ] );
                        foreach( $admin_calendly_data as $value ){
                            $checkData = getQuery( ['table' => 'calendly_event' , 'where' => [ 'user_id' => $user['id'] , 'event_name' => $value['event_name'] ] ] );
                            if(empty( $checkData )){
                                $calendly_data[] = $value;
                            }
                        }
            ...
            $data['counsling_booking_url']= 'https://calendly.com/'.$booking_link;
            //load view
            $this->load->view('navbar',$data);
            $this->load->view('user/sidebar',$data);
            $this->load->view('user/counseling_parameters_view',$data);
            $this->load->view('footer');
        }
        public function notifications()
        {
            // echo "Hi";die;
            $data= $this->initializer();
            $data['ap_book_data'] = getQuery([ 'where' =>  [ 'email' => $data['user']['email'] ] , 'table' => 'user_book_appointment' ,  'order' => [ 'id' => 'DESC']]);
            // lQ( 1 );
            $this->load->view( 'navbar', $data );
        ...
        public function  rating_review($id, $for){
            $data = $this->initializer();
            $notification = getQuery( [ 'where' => [ 'id' => $id ] , 'table' => 'user_book_appointment' , 'single' => true ] );
            if( !empty($notification) ){
                $data['editData'] = $notification;
            }
            else{
                $data['assessment_code'] = $id;
            }
            $data['for'] = $for;
            // pre( $data ,  1);
            $this->load->view('navbar',$data);
            $this->load->view('user/sidebar'); 
            $this->load->view('user/rating_review',$data); 
            $this->load->view('footer');
        }
        public function update_review(){
            ...
            if($this->form_validation->run() == true)
            {
                if( isset( $_POST['assessment_code'] ) ){
                    // pre( $_POST );
                    $notificationData[ 'user_id' ] = $data['reseller_sp']['id'];
                    $notificationData[ 'assessment_code' ] = $_POST['assessment_code'];
                    $notificationData[ 'appointment_type' ] = 'assessment_review';
                    $notificationData[ 'name' ] = $data['user']['fullname'];
                    $notificationData[ 'email' ] = $data['user']['email'];
                    $notificationData[ 'phone_no' ] = $data['user']['mobile'];
                    $notificationData[ 'message' ] = 'Review For Assessment';
                    $notificationData[ 'status' ] = 'completed';
                    $notificationData[ 'created_at' ] = date('Y-m-d H:i:s');
                    insert( 'user_book_appointment' , $notificationData );
                    $_POST['notificationId']  = $this->db->insert_id();
                    unset( $_POST['assessment_code'] );
                    $_POST[ 'user_id' ] = $data['user']['id'];
                    // pre( $_POST );
                    // die;
                    insert( 'review_rating' , $_POST );
                    if( affected() > 0 ){
                        $rating = getQuery( [ 'select' => 'count(id) as id, SUM(rating) as total_rating', 'where' => [ 'reseller_id' => $data['reseller_sp']['id'] ] , 'table' => 'review_rating' , 'single' => true ]);
                        $rating_data = round($rating['total_rating']/$rating['id']);
                        update( 
                            [ 'where' => [ 'id' => $data['reseller_sp']['id'] ] , 'data' => ['rating_value_automated' => $rating_data ] , 'table' => 'user_details' ] 
                            );
                        if( affected()  > 0){
                            $this->session->set_flashdata('msg','Review Has been updated');
                            $this->session->set_flashdata('status','success');
                            redirect(base_url().'/BaseController/notifications');
                        }
                        else{
                            $this->session->set_flashdata('msg','Update Remains Same, try again');
                            $this->session->set_flashdata('status','info');
                            redirect(base_url().'/BaseController/notifications');
                        }
                    }
                    else{
                        $this->session->set_flashdata('msg','Not Update Try Again');
                        $this->session->set_flashdata('status','success');
                        redirect(base_url().'/BaseController/'. $_POST['notificationId'].'/services');
                    }
                }
                else{
                    // pre($_POST, 1);
                    insert( 'review_rating' , $_POST );
                    if( affected() > 0 ){
                        $rating = getQuery( [ 'select' => 'count(id) as id, SUM(rating) as total_rating', 'where' => [ 'reseller_id' => $_POST['reseller_id'] ] , 'table' => 'review_rating' , 'single' => true ]);
                        $rating_data = round($rating['total_rating']/$rating['id']);
                        update(
                            [ 'where' => [ 'id' => $_POST['reseller_id'] ] , 'data' => ['rating_value_automated' => $rating_data ] , 'table' => 'user_details' ] 
                            );
                        if( affected()  > 0){
                            $this->session->set_flashdata('msg','Review Has been updated');
                            $this->session->set_flashdata('status','success');
                        ...
                        $this->session->set_flashdata('msg','Not Update Try Again');
                        $this->session->set_flashdata('status','success');
                        redirect(base_url().'/BaseController/'. $_POST['notificationId'].'/counselling');
                    }
                }
            }
            else
            {
                $this->session->set_flashdata('msg','Fill Form Correctly');
                    $this->session->set_flashdata('status','success');
                    redirect(base_url().'/BaseController/'. $_POST['notificationId'].'/counselling');
            }
        }
        ...
        public function request_code()
        {
            ...
            $data['resellerVariation'] = getQuery( [ 'where' => [ 'reseller_id' => $data['reseller_sp']['id'] ] , 'table' => ' reseller_assessment_variation' , 'single' => true ]  );
            
            $data['resellerVariation'] = getQuery( [ 'where' => [ 'reseller_id' => $data['reseller_sp']['id'] ] , 'table' => ' reseller_assessment_variation' , 'single' => true ]  );

            $data['segment'] = getQuery( [ 'select' => 'mkt_sgmt' , 'where' => [ 'status' => 'active' ] , 'table' => 'solution_variant_new' , 'group_by' => [ 'mkt_sgmt' ] ] );
            
            // echo "SELECT DISTINCT Cluster FROM c_library INNER JOIN c_library_1 ON c_library.standard_title = c_library_1.profession_name WHERE status_one='active' and cdb_status='added' and name_map != 'to_map' and c_library_1.status = 1 order by Cluster "."<br>";
            
            $data['cluster'] = getQuery( [ 'select' => 'Cluster' , 'table' => ' career_edu_v2' , 'group_by' => [ 'Cluster'  ]  ] );



            
            $data['stream'] = getQuery( [ 'select' => '11th_12th' , 'table' => ' career_edu_v2' , 'group_by' => [ '11th_12th'  ]  , 'where' => [ '11th_12th !=' => 'Any Stream'  ] ] );
            //  lQ(1);
        ...
        function variant_filter()
        {
            if( !empty( $_POST ) ){
                extract( $_POST );
                $data = getFilter( $searchColumn , $value , $selectColumn );
                // lQ();
                // pre( $_SESSION['variant_filter_arr'] , 1);
                $reportNature = 'No Data Found';
                $reportType = 'No Data Found';
                $list = 'no list';
                if( !empty( $data['list'] ) ){
                    if( !isset( $data['price'] ) ){
                        ...
                        foreach($data['list'] as $key => $value){
                            $selectData.='<option value="'.$value[$selectColumn].'">'.ucwords(str_replace('_' , ' ' , $value[$selectColumn])).'</option>';
                            $reportNature.='<div> <h5>'.ucwords($value[$selectColumn]).'</h5><p>'.ucfirst($value['report_nature_inf']).'</p></div>';
                            if( $selectColumn == 'report_type' ){
                                $reportType .= '<div> <h5>'.ucwords($value[$selectColumn]).'</h5><p>'.ucfirst($value['report_type_inf']).'</p></div>';
                            }
                        }
                        $list = 'list';
                    }
                    else{
                        $selectData = [ 'price' => $data['list']['mrp_reseller'] , 'variantId' => $data['list']['id'] ];
                    }
                }
                else{
                    $selectData = '<option value="">Select</option><option value="na">Default</option>';
                    $list = 'no list';
                }
                $response = [ 'status' => 'success' , 'code' => '201', 'message' => 'Request Valid' , 'data' => $selectData , 'reportNature' => $reportNature , 'reportType' => $reportType , 'list' => $list ];
            }
            else{
                $response = [ 'status' => 'failed' , 'code' => '404', 'message' => 'Invalid Request , Data Not Found' ];
            }
            echo json_encode( $response );
        }

        function variant_form_submit()
        {
            if( !empty( $_POST ) ){
                $user = $this->session->userdata('user');
                $data = $this->initializer();
                // pre( $_POST , 1);
                extract( $_POST );
                $_SESSION['variant_form_data'] = [
                    'variationId' => $variantId,
                    'userId' => $user['id'],
                    'cluster' => (isset($cluster) ? $cluster  : '' ),
                    'profession' => ( isset($profession) ? $profession : '' ),
                    'currentStream' => ( isset( $currentStream) ? $currentStream : '' ),
                    'changeStream' => ( isset( $changeStream ) ? ( $changeStream != '' ? json_encode( $changeStream )  : '' ) : '' )
                ];
                $response =  [ 'status' => 'success' , 'code' => 200  , 'message' => 'data found' ];
            }
            else{
                $response =  [ 'status' => 'failed' , 'code' => 300 , 'data' => [] , 'message' => 'Invalid Request' ];
            }
            echo json_encode( $response );
        }
        ...
        function get_profession(){
            if( !empty( $_POST ) ){
                $proffessionList = '<option value="">select</option>';
                $data = getQuery( [ 'select' => 'profession_name , indian_name' ,'order_by' => [ 'profession_name' => 'ASC' ] , 'where' => [ 'Cluster' => $_POST['cluster'] ] , 'table' => 'career_edu_v2' , 'subquery' => [ 
                        'col' => 'profession_name',
                        'con' => ' IN ',
                        'query' => '( SELECT profession_name FROM career_apt_v2 WHERE Cluster = "'.$_POST["cluster"].'" )'
                    ] ] );
                // lQ(1);
                if( !empty( $data ) ){
                    foreach($data as $key => $value ){
                        $proffessionList .= '<option value="'.$value['profession_name'].'" >'.( $value['indian_name'] != '' ? $value['indian_name'] : $value['profession_name'] ).'</option>';
                    }
                }
                $response =  [ 'status' => 'success' , 'code' => 200 , 'data' => $proffessionList ];
            }
            else{
                $response =  [ 'status' => 'failed' , 'code' => 300 , 'data' => [] , 'message' => 'Invalid Request' ];
            }
        ...
        public function user_request_code($email,$r_email,$solution,$sl_name,$variation=null)
        {   date_default_timezone_set("Asia/Kolkata");
        ...
        public function view_code($codeData=null)
        {
            ...
            $code['s'] = getQuery( [ 'result' => true , 'where' => [ 'user_id' => $user['email'] , 'status !=' => 'pending' ] , 'order' => [ 'id' => 'DESC' ] , 'table' => 'user_code_list' , 'limit' => [ 10 => 0 ] ] );
            
            $code['total'] = getQuery( [ 'single' => true , 'select' => 'COUNT(id) as codes' , 'where' => [ 'user_id' => $user['email'] , 'status !=' => 'pending' ] , 'order' => [ 'id' => 'DESC' ] , 'table' => 'user_code_list' ] );
        ...
        public function filter_list($currentPage)
        {
            if( $currentPage == 1 ){
                $from = 0;
            }
            ...
            $code['allowed_services'] = explode(",",$data['reseller_sp']['allowed_services']);
            $code['assessment_expiry_window'] = $data['reseller_sp']['assessment_expiry_window'];
            $code['expiry_applicable_to'] = explode(",",$data['reseller_sp']['expiry_applicable_to']);
            $code['reportData'] = $this->Admin_model->getReportConfig();
            $code['reportControl'] = $this->User_model->getreportcontrol($user['user_id']);
            // $code['s'] = $res = $this->Base_model->code_list($user['email']);
            $code['s'] = getQuery( [ 'result' => true , 'where' => [ 'user_id' => $user['email'] , 'status !=' => 'pending' ] , 'order' => [ 'id' => 'DESC' ] , 'table' => 'user_code_list' , 'limit' => [ 10 => $from ] ] );
            ...
            // $this->load->view('navbar',$data);
            // $this->load->view('user/sidebar');
            $this->load->view('user/filter_list_code',$code); //replace with view_approve_code 
            // $this->load->view('footer');
            // $this->session->unset_userdata("ref_data");      
            // pre( $code , 1 );
            // die;
        ...
        public function user_training()
        {
            ...
            // pre( $data["trainingDetail"]  , 1);
            $data["apply_training"] = $this->sp_vocation_training_info("APPLY_TRAINING");
            $data["trainingDetail"] = getQuery( [ 'where' => [ 'user_email' => $user['email'] , 'trainingType' => 'live' ] , 'table' => 'vocational_training_apply_user' , 'join' => [ [ 'vocational_live_training' , ' vocational_live_training.id =  vocational_training_apply_user.training_id' , 'INNER' ] ] , 'order' => [ 'vocational_training_apply_user.id' => 'DESC' ] ] );
            
            $data["sessionDetail"] = getQuery([ 'select' => 'vocational_live_session.*', 'where' => [ 'vocational_live_session.status !=' => 'inactive' , 'vocational_live_session.status !=' => 'canceled' ,'user_email' => $user['email'], 'trainingType' => 'live' ] , 'table' => 'vocational_training_apply_user' , 'join' => [ [ 'vocational_live_training' , ' vocational_live_training.id =  vocational_training_apply_user.training_id' , 'INNER' ], [ 'vocational_live_session' , ' vocational_live_session.trainingId = vocational_live_training.id' , 'LEFT'] ] , 'order' => [ 'vocational_live_session.id' => 'ASC' ] ]);
            // pre( $data["sessionDetail"] , 1 );
            $this->load->view('navbar',$data);
            $this->load->view('user/sidebar',$data); 
        ...
        function update_session_status( $sessionId , $status ){
            if( is_numeric($sessionId) ){
                $user = $this->session->userdata('user');
                if( $status == 'absent' || $status == 'present' || $status == 'completed' ){
                    $sessionData = getQuery( [ 'where' => [ 'sessionId' => $sessionId ] , 'table' => 'vocational_session_user_status' , 'single' => true ] );
                    if( !empty( $sessionData ) ){
                        update( ['table' => ' vocational_session_user_status' , 'data' => ['status' => $status ] , 'where' => [ 'sessionid' => $sessionId ] ] );
                        if( affected() > 0 ){
                            setFlashData(['msg'=>'Status Has been updated','status'=> 'success'],'BaseController/user_training');
                        }
                        else{
                            setFlashData([ 'msg' => 'Update Remains same','status' => 'success'],'BaseController/user_training');
                        }
                    }
                    else{
                        $sessionData = getQuery( [ 'where' => [ 'id' => $sessionId ] , 'table' => 'vocational_live_session' , 'single' => true ] );
                        if( !empty( $sessionData ) ){
                            $id=insert( 'vocational_session_user_status',
                                    [
                                        'user_id' => $user['id'],
                            ...
                            if( $id ){
                                setFlashData( [ 'msg' => 'Session Status Has been updated','status' => 'success' ] , 'BaseController/user_training' );
                            }
                            else{
                                setFlashData( [ 'msg' => 'Session Status Not Updated, Try Again','status' => 'info' ],'BaseController/user_training' );
                            }
                        }
                        else{
                            setFlashData( [ 'msg' => 'Session Not Found','status' => 'warning'],'BaseController/user_training');
                        }
                    }
                }
                else{
                    setFlashData( [ 'msg' => 'Invalid Parameter Session Status','status' => 'warning'],'BaseController/user_training');
                }
            }
            else{
                setFlashData( [ 'msg' => 'Invalid Parameter Session Id','status' => 'warning'],'BaseController/user_training');
            }
        }

        function update_rating_review( $trainingId ){
            if( is_numeric( $trainingId ) ){
                $trainingDetail = getQuery( [ 'where' => [ 'training_id' => $trainingId , 'trainingType' => 'live' ] , 'table' => 'vocational_training_apply_user' , 'single' => true ] );
                if( !empty( $trainingDetail ) ){
                    $user = $this->session->userdata('user');
                    $data = $this->initializer();
                    $data['user'] = $user;
                    $data['trainingDetail'] = $trainingDetail;
                    $data['sessionList'] = getQuery( [ 'select' => 'count(id) as total' , 'where' => [ 'trainingId' => $trainingId , 'status !=' => 'inactive' , 'status !=' => 'canceled' ] , 'table' => 'vocational_live_session', 'single' => true ] );
                    $data['sessionStatus'] = getQuery( [ 'where' => [ 'trainingId' => $trainingId , 'status' => 'completed' ] , 'table' => 'vocational_session_user_status'  ] );;
                    view( 'navbar',  $data);
                    view( 'user/sidebar', $data); 
                    view( 'user/training_review' , $data); 
                    view( 'footer' );
                }
                else{
                    setFlashData( [
                        ...
                        'BaseController/user_training',
                    );
                }
            }
            else{
                setFlashData( [
                        'msg' => 'Invalid Training Id',
                        'status' => 'warning'
                    ],
                    'BaseController/user_training',
                );
        ...
        function training_update_review(){
            if( !empty( $_POST ) ){
                // pre( $_POST , 1 );
                $user = $this->session->userdata('user');
                $resellerData = getQuery( [ 'where' => [ 'user_id' => $user['user_id'] , 'iam' => 'sp'] , 'or_where' =>[ 'iam' => 'reseller' ] , 'table' => 'user_details' , 'single' => true ] );
                // lQ(1);
                $data = $this->initializer();
                insert( 'review_rating' , [
                    'user_id' => $user['id'],
                ...
                update(  [ 'table' =>  'vocational_training_apply_user' , 'data' => [ 'training_status' => $_POST['status'] ] , 'where' => [ 'id' => $_POST['applyId'] ] ] );
                if( affected() > 0 ){
                    setFlashData( [ 'msg' => 'Training review has been updated' , 'status' => 'info' ] , 'baseController/user-training' );
                }
                else{
                    setFlashData( [ 'msg' => 'Training review not updated, try again' , 'status' => 'info' ] , 'baseController/user-training' );
                }
            }
            else{
                setFlashData( [ 'msg' => 'Invalid Request' , 'status' => 'warning' ] , 'baseController/user-training' );
            }
        }
        ...
        function user_request_code_online( $email,$r_email,$solution_id,  $status = null , $variation = null){
            if( $status != null ){
            ...
            if( $variation != null ){
                $mrp_data = getQuery( [ 'where' => [ 'id' => $_SESSION['variant_form_data']['variationId'] ] , 'table' => 'solution_variant_new' , 'single' => true ] );
                // if( empty( $user ) ){
                    $order_amount = $mrp_data['mrp_reseller']*100;
                // }
            }
            else{
                if( $sp_details->email == "merak@gmail.com" ){
            ...
            if( $variation != null ){
                redirect(base_url().'payment/checkout/variaton');
            }
            else{
        ...
        {   $sol_disp_name ='';
            ...
            $db_tbl = $this->config->item('asmt_map')['sol_tbl'][$sol][$part];
            // pre( $this->config->item('asmt_map') , 1 );
        ...
        private function assessment_analysis($part,$sol,$code)
        {
            ...
            // pre( [ $solution, $solution_table, $options_within_table, $has_right_answer, $option_field_names,$question_group,$code,$solution_db,$sol ] , 1 );
