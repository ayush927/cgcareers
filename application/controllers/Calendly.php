...
class Calendly extends CI_Controller{
    ...
    function user_request_code_online($id, $email, $r_email){
        ...
        if( empty( $solution_details ) ){
            $solution_details = $this->User_model->get_user_details("RESELLER_EVENT_DETAIL",$purchase_code_details['id']);
        }
        // lQ(1);
        // die;
        // pre( $solution_details);
    ...
    function checkout(){
        ...
        if( empty( $solution_details ) ){
            $solution_details = $this->User_model->get_user_details("RESELLER_EVENT_DETAIL",$purchase_code_details['solution_id']);
        }
    ...
    function confirmation(){
        ...
        if( !empty($user) ){
            // pre( $_REQUEST , 1 );
            $user_details = $this->User_model->get_user_details( "DETAILS_BY_USER_ID", $user['user_id']);
            ...
            $body_msg  = "Dear ".$user_details['fullname']." <br/><br/> This is mail from respicite for appointment booked By 
                ...
                <strong> Counsellor Name : ".$user_details['fullname']."</strong> <br>
            ...
            $body_msg  = "Dear ".$data['name']." <br/><br/> This is mail from respicite for appointment booked Detail
                <br>
                <strong> Name : ".$data['name']."</strong> <br>
                <strong> Counsellor Name : ".$user_details['fullname']."</strong> <br>
                <strong> Email : ".$data['email']."</strong> <br>
                ...
                <br/> Team Respicite <br/> <a href='https://respicite.com'>https://respicite.com</a> ";
            // pre( $body_msg );
            // pre( $user_details['email'] );
            // pre( $_GET['invitee_email'] , 1 );
            
            $this->User_model->otp_send_on_email($_GET['invitee_email'],$subject,$body_msg);
            // die;
            $this->db->insert("user_book_appointment",$data);
            redirect('BaseController/notifications');
        }
        else{
        // echo "Hello";
            $purchase_code_details = $this->session->userdata("purchase_code_details");
        // pre( $purchase_code_details );
        // pre( $_REQUEST );
            ...
            if( !empty( $purchase_code_details ) ){
                ...
                $body_msg  = "Dear ".$data['name']." <br/><br/> This is mail from respicite for appointment booked Detail 
                    ...
                    <br/> Team Respicite <br/> <a href='https://respicite.com'>https://respicite.com</a> ";
                // pre( $body_msg );
                // pre( $purchase_code_details['r_email'] );
                $this->User_model->otp_send_on_email($purchase_code_details['r_email'],$subject,$body_msg);
                $body_msg  = "Dear ".$user_details['fullname']." <br/><br/> This is mail from respicite for appointment booked By 
                    <br>
                    <strong> Name : ".$data['name']."</strong> <br>
                    <strong> Counsellor Name : ".$user_details['fullname']."</strong> <br>
                ...
                // pre( $body_msg );
                // pre( $data['email'] , 1 );
                $this->User_model->otp_send_on_email( $data['email'],$subject,$body_msg);
                // die;
                $this->db->insert("user_book_appointment",$data);
