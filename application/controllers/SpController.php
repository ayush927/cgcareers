...
class SpController extends CI_Controller
{
    ...
    public function initializer()
    {
        ...
        $data["reseller_sp"] = $this->Commen_model->get_reseller_sp(
            $data["user"]["user_id"]
        );
        $this->db->select("iam");
        ...
        $data["mot_menu"] = $this->Commen_model->get_mot_menu(
            "sp",
            "mot_sidebar"
        );
        $data["mot_navbar"] = $this->Commen_model->get_mot_menu(
            "sp",
            "mot_navbar"
        );
    ...
    public function edit_sp_profile()
    {
        ...
        if (isset($_POST["updatebtn"])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $this->db->set("profile_link", $_POST["profile_link"]);
                $this->db->where("email", $email);
                $this->db->update("user_details");
                ...
                redirect(
                    base_url() . "SpController/edit_sp_profile",
                    "refresh"
                );
            } else {
    ...
    function variation_image_list()
    {
        ...
        $userList = getQuery([
            "select" =>
                "solution_variant.* , solution_variation_data.created_date , solution_variation_data.imageName , solution_variation_data.updated_date , solution_variation_data.id as dataId , solution_variation_data.variationId",
            "table" => "solution_variation_data",
            "join" => [
                [
                    "solution_variant",
                    "solution_variant.id = solution_variation_data.variationId",
                    "INNER",
                ],
            ],
        ...
        if (!empty($userList)) {
            foreach ($userList as $k => $v) {
                $where_not_in_numeric["solution_variation_data.variationId"][] =
                    $v["variationId"];
            }
        ...
        $this->load->view("sp/variation-image-list", $data);
    ...
    function delete_variation_image($deleteId)
    {
        ...
        if (affected() > 0) {
            ...
            redirect("SpController/variation-image-list");
        } else {
            $this->session->set_flashdata("msg", "please try again");
            redirect("SpController/variation-report-image");
        }
    }
    ...
    function variation_report_image($id = null)
    {
        ...
        if ($id != null) {
            $data["edit"] = getQuery([
                "select" =>
                    "solution_variant.* , solution_variation_data.created_date , solution_variation_data.imageName , solution_variation_data.updated_date , solution_variation_data.id as dataId",
                "table" => "solution_variation_data",
                "join" => [
                    [
                        "solution_variant",
                ...
                "where" => ["solution_variation_data.id" => $id],
                "single" => true,
            ]);
            if (!empty($data["edit"])) {
                // pre( $data['edit']  , 1);
        ...
        $data["getsolutions"] = getQuery([
            "select" => "solution_name",
            "table" => " solution_variant",
            "group_by" => ["solution_name"],
        ]);

        if (isset($_GET["solution_name"])) {
            $data["getClass"] = getQuery([
                "table" => " solution_variant",
        ...
        $this->load->view("sp/add_variation_image", $data);
        ...
        if (!empty($_POST)) {
            ...
            if ($_FILES["image"]["name"] != "") {
                ...
                if (empty($getData)) {
                    ...
                    unlink(
                        "uploads/solution-variation/" . $getData["imageName"]
                    );
                    update([
                        "where" => [
                            "user_id" => $user["id"],
                    ...
                    if (affected() > 0) {
                        ...
                        $this->session->set_flashdata(
                            "msg",
                            "data remains same , please check again"
                        );
                        redirect("SpController/variation-image-list");
                    }
                }
                ...
                if (
                    ...
                    if (
                        ...
                        if (
                            ...
                            $this->session->set_flashdata(
                                "msg",
                                "Sorry You Select Same Combination"
                            );
                            redirect("/SpController/l4", "refresh");
                        }
                        ...
                        $this->db->where($where);
                        $cnt = $this->db
                            ->get("provider_detail_four")
                            ->num_rows();
                        if ($cnt > 0) {
                            $ecode = $i;
                            $this->session->set_flashdata(
                                "msg",
                                "Combination $ecode Already Exist"
                            );
                            redirect("/SpController/l4", "refresh");
                        } else {
                            $status = "ok";
                        ...
                        $this->session->set_flashdata(
                            "msg",
                            "Please Fill All Details in combination $ecode."
                        );
                        redirect(base_url() . "/SpController/l4");
                    }
                }
    ...
    public function do_upload()
    {
        ...
        if (isset($_FILES["img"])) {
            ...
            if (file_exists("./uploads/original/" . $new_name)) {
                unlink("./uploads/original/" . $new_name);
            }
            ...
            if (!$this->upload->do_upload("img")) {
                ...
                resize_image(
                    ...
                    100,
                    100,
                    "uploads/counsellor-thumb/" . $new_name
                );
                $user = $this->session->userdata("user");
                $data["user"] = $user;
                $email = $user["email"];
            ...
            $this->session->set_flashdata("msg2", "Please select Image");
            redirect(base_url() . "SpController/spprofile", "refresh");
        }
    }
    ...
    public function onboarding_exam($grp)
    {
        ...
        if ($grp == "Career Explorer") {
            redirect(
                base_url() .
                    "/SpController/career_explorer_onboarding_test/" .
                    $ob_no_ce
            );
        } elseif ($grp == "Positive Parenting") {
            redirect(
                base_url() . "/SpController/positive_parenting_onboarding_test"
            );
        } elseif ($grp == "Career Excellence") {
            $ob_no_ce = 1;

            redirect(
                base_url() .
                    "/SpController/career_excellence_onboarding_test/" .
                    $ob_no_ce
            );
        } elseif ($grp == "Career Builder") {
            $ob_no_ce = 1;

            redirect(
                base_url() .
                    "/SpController/career_builder_onboarding_test/" .
                    $ob_no_ce
            );
            ...
            redirect(
                base_url() .
                    "/SpController/overseas_companion_onboarding_test/" .
                    $ob_no_ce
            );
        }
        ...
        if (isset($_POST["saveBtn"])) {
            if ($num != $count_value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect(
                        base_url() .
                            "SpController/positive_parenting_onboarding_test"
                    );
                } else {
                    $this->session->set_flashdata("msg", validation_errors());
        ...
        if (isset($_POST["saveBtn"])) {
            if ($num != $count_value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect(
                        base_url() .
                            "SpController/score_calculation/cbot/" .
                            $ob
                    );
                } else {
                    $this->session->set_flashdata("msg", validation_errors());
        ...
        if (isset($_POST["saveBtn"])) {
            if ($num != $count_value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect(
                        base_url() .
                            "SpController/score_calculation/cexot/" .
                            $ob
                    );
                } else {
                    $this->session->set_flashdata("msg", validation_errors());
        ...
        if (isset($_POST["saveBtn"])) {
            if ($num != $count_value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect(
                        base_url() .
                            "SpController/score_calculation/" .
                            $solution .
                            "/" .
                            $ob
                    );
        ...
        if (isset($_POST["saveBtn"])) {
            if ($num != $count_value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect(
                        base_url() .
                            "SpController/overseas_companion_certification_test/" .
                            $ob
                    );
                } else {
                    $this->session->set_flashdata("msg", validation_errors());
    ...
    public function score_calculation($grp, $ob)
    {
        ...
        $this->session->set_flashdata(
            "msg",
            "Assessment Compeleted Please Wait for Report"
        );
    ...
    public function ce_exam($grp)
    {
        ...
        if ($grp == "Career Explorer") {
            ...
            redirect(
                base_url() .
                    "/SpController/career_excellence_ce_test/" .
                    $ob_no_ce
            );
        } elseif ($grp == "Career Builder") {
            $ob_no_ce = 1;
            ...
            redirect(
                base_url() . "/SpController/career_builder_ce_test/" . $ob_no_ce
            );
        } elseif ($grp == "Overseas Companion") {
            $ob_no_ce = 1;
            ...
            redirect(
                base_url() .
                    "/SpController/overseas_companion_ce_test2/" .
                    $ob_no_ce
            );
        }
        ...
        if (isset($_POST["saveBtn"])) {
            if ($num != $count_value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect(
                        base_url() . "SpController/positive_parenting_ce_test"
                    );
                } else {
                    $this->session->set_flashdata("msg", validation_errors());
        ...
        if (isset($_POST["saveBtn"])) {
            if ($num != $count_value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect(
                        base_url() .
                            "SpController/career_builder_ce_test/" .
                            $ob_no
                    );
                } else {
                    $this->session->set_flashdata("msg", validation_errors());
    ...
    public function page_change_logo()
    {
        ...
        if (isset($_POST["savebtn"])) {
            ...
            if (!$this->upload->do_upload("img")) {
                ...
                $data[
                    "allowed_services"
                ] = $this->Admin_model->getUserDetailsById($user["id"]);
    ...
    public function footer_field()
    {
        ...
        if (isset($_POST["updatebtn"])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $q_check = $this->db->get_where("day_description", [
                    "email" => $email,
                    "day" => $_POST["Todays_Day"],
                ]);
        ...
        if ($code != "" && $solution != "" && $user_id != "") {
            ...
            if ($id) {
                if (empty($getUserVariation)) {
                    ...
                    $getUserVariation["report_variation"] == "two" ||
                    $getUserVariation["report_variation"] == "three"
                ) {
                    echo 3;
                    redirect(
                        base_url() .
            ...
            case "FORWARD_DETAILS":
                ...
                if (!empty($request_user_id)) {
                    if (!empty($forward_email_id)) {
                        ...
                        $data_template[
                            "user_details"
                        ] = $this->Sp_model->get_job_data(
                            "JOB_REQUEST_USER",
                            $request_user_id
                        );

                        $msg_body = $this->load->view(
                            "sp/mail_template/forward_user_details.php",
                            $data_template,
                            true
                        );

                        if (
                            $this->Sp_model->send_on_email(
                                $forward_email_id,
                                $subject,
                                $msg_body
                            )
        ...
        if ($this->form_validation->run()) {
            $data = [
                ...
                "available_days" => implode(
                    ",",
                    $this->input->post("available_days")
                ),
                ...
                "counselling_sessions" => $this->input->post(
                    "counselling_sessions"
                ),

                "top_skills" => implode(",", $this->input->post("top_skills")),

                "most_relevant_education" => $this->input->post(
                    "most_relevant_education"
                ),
    ...
    public function marketplace_new($num = null)
    {
        ...
        if (empty($_POST)) {
            ...
            $config["total_rows"] = count(
                $this->Admin_model->fetch_user_details("", "", $key3)
            );
        }
    ...
    public function addLandingSectionParameter()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($result) {
                ...
                redirect(
                    "/SpController/landingsPages/" . $landingId,
                    "refresh"
                );
            } else {
                $this->session->set_flashdata("msg", "Opps Unable to add data");
    ...
    public function update_book_link()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $updateData = $this->User_model->book_link_update(
                $user["id"],
                $book_link
            );
    ...
    public function counselingUpdate()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($update) {
                ...
                redirect(
                    base_url() . "SpController/counselingParameters",
                    "refresh"
                );
            } else {
                $this->session->set_flashdata("msg  ", "something wrong");
    ...
    public function insertAssociate()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $code = random_strings();
            $user = $this->session->userdata("user");
            $num_rows = $this->Sp_model->get_num_rows_for_associate(
                $user["user_id"]
            );
            $next_num = (int) $num_rows + 1;
            $arr_u_id = ["ecom" => "iquery"];
            if (!isset($arr_u_id[$user["user_id"]])) {
                $uid = $user["user_id"];
            } else {
            ...
            $formData = [
                ...
                "associate_type" => $this->input->post("associate_type")
                    ? $this->input->post("associate_type")
                    : "",
            ...
            if ($result) {
                ...
                $body_msg =
                    ...
                    $this->input->post("email") .
                    " <br/> <br/> 
                Please complete your registration with Respicite
                by using the following password - <b>" .
                    $password .
                    "</b><br/>
                ...
                $this->session->set_flashdata(
                    "msg",
                    "Mail has been send , Added Successfully"
                );
                redirect("/SpController/getAllAssociate/", "refresh");
            } else {
                setFlashData([
                    "status" => "info",
                    "msg" => "Opps Unable to add data",
                ]);
                redirect("/SpController/getAllAssociate/", "refresh");
            }
        } else {
            setFlashData([
                "status" => "info",
                "msg" => "This Email Id is already registered with us",
            ]);
            redirect("/SpController/addAssociate/", "refresh");
        }
    }
    ...
    public function insert_selected_code()
    {
        ...
        $result = $this->Sp_model->insert_selected_code(
            $selected_code,
            $email,
            $user_id,
            $user_type
    ...
    public function getAllAssociate()
    {
        ...
        $db_data["associate_data"] = $this->Sp_model->getAssociate(
            $user_id,
            $user_type
        );
    ...
    function change_status($id, $status)
    {
        ...
        if (affected() > 0) {
            ...
            redirect("SpController/getAllAssociate");
        } else {
            setFlashData([
                "msg" => 'Status Remain\'s Same',
                "status" => "info",
            ]);
            redirect("SpController/getAllAssociate");
        }
    ...
    public function updateAssociate($id)
    {
        ...
        $formData["associate_type"] = $this->input->post("associate_type")
            ? $this->input->post("associate_type")
            : "";
        ...
        $formData["code"] = $this->input->post("code")
            ? $this->input->post("code")
            : "";

        $formData["selected_code"] = $this->input->post("selected_code")
            ? $this->input->post("selected_code")
            : "";
