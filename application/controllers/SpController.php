...
class SpController extends CI_Controller
{
    ...
    public function do_upload()
    {
            ...
            if( isset($_FILES['img']) ){
                ...
                {
                    resize_image( 'uploads/original/'.$new_name , 300 , 300 , 'uploads/'.$new_name );
...

    {
            ...
            if( $user['iam'] == 'reseller' ){                 $this->load->view('sidebar', $data);             }             else{                 $this->load->view('sp/sidebar',$data);             } 
...
public function section_via_parameter()
    ...
    function change_status( $id  , $status){
        update( [ 'data' => [ 'status' => $status ] , 'where' => [ 'id' => $id ] , 'table' => 'user_associate' ] );
        if( affected() > 0 ){
            setFlashData( [ 'msg' => 'Status Has been updated' , 'status' => 'success' ] );
            redirect('SpController/getAllAssociate');
        }
        else{
            setFlashData( [ 'msg' => 'Status Remain\'s Same' , 'status' => 'info' ] );
            redirect('SpController/getAllAssociate');
        }
    }
