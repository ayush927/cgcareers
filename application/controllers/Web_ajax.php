...
class Web_ajax extends CI_Controller{
            ...
            switch($action){
                case "GET_DETAILS":
                    ...
                    if( isset( $_GET['certification'] ) ){
                        $this->db->where('certification' , $_GET['certification'] );
                    }
                    if( isset( $_GET['domainId'] ) ){
                        $this->db->where('domainId' , $_GET['domainId'] );
                    }
                    if( isset( $_GET['subDomainId'] ) ){
                        $this->db->where('subDomainId' , $_GET['subDomainId'] );
                    }
                    if( isset( $_GET['language'] ) ){
                        $this->db->where('language' , $_GET['language'] );
                    }
                    if( isset( $_GET['nature'] ) ){
                        $this->db->where('nature' , $_GET['nature'] );
                    }
                    if( isset( $_GET['certification'] ) ){
                        $this->db->where('certification' , $_GET['certification'] );
                    }
                    $this->db->limit( $limit, $offset*$limit);
                    $q = $this->db->get("vocational_live_training");
                    $q = $q->result_array();
                    // lQ(1);
                    if(!empty($q)){
                        $filter_data = [];
                        foreach($q as $v){
                            $filter_data[] = [
                                "t_id"                        =>  $v['id'],
                                "sp_id"                     =>  $v['sp_id'],
                                "training_img"              => base_url("uploads/vocational_training/").(($v['image'] != null) ? $v['image']: "default-training-img.jpeg"), 
                                "training_name"             =>  word_limiter($v['training_name'],15),
                                "training_desc"             =>  word_limiter($v['short_descriptoin'],150),
                            ];
                        }
                        $json_msg["msg"] = "DATA_OK";
                        $json_msg["data"] = $filter_data;
                ...
                case "FULL_DETAILS":
                    ...
                    if(!empty($q)){
                        $filter_data["training"] = [
                            ...
                            "fullname"                  =>  $q['fullname'],
                            "training_img"              => base_url("uploads/vocational_training/").(($q['image'] != null)? $q['image']: "default-training-img.jpeg"),
                        ...
                        if( $userData['iam'] == 'user' ){
                            ...
                            if( isset( $_GET['scode'] ) ){
                                ...
                                if($this->User_model->assessment_otp_update_by_email($user_email,$OTP_code) > 0){
                                    $subject = "Respicite notification - action required";
                                    $body_msg  = emailTemplate([ 'type' => 'assessment' , 'email' =>  $user_email , 'solutionName' => $solutionData['solution'] , 'otp' => $OTP_code  ]);
                            ...
                            elseif( !isset( $_GET['scode']) && !isset( $_GET['pcode']) ){
                                // echo "Hello1";
                                $where = "user_id='$user_email' and status!='pending' and status = 'Ap'";
                                $this->db->where($where);
                                $row = $this->db->get('user_code_list')->result_array();
                                $where = "user_id='$user_email' and status!='pending' and status = 'Rp'";
                                $this->db->where($where);
                                $row1 = $this->db->get('user_code_list')->result_array();
                                if( !empty( $row ) ){
                                    $json_msg["msg"] = "LOGIN";
                                    $json_msg['solutionList'] = $row;
                                    $json_msg['completedList'] = $row1;
                                }
                                else{
                                    $json_msg["msg"] = "LOGIN";
                                    $json_msg['solutionList'] = !empty( $row ) ? $row : [] ;
                                    $json_msg['completedList'] = !empty( $row1 ) ? $row1 : [] ;
                                    $OTP_code = rand(1000,1000000);
                                    if($this->User_model->assessment_otp_update_by_email($user_email,$OTP_code) > 0){
    ...
    public function get_details( $user_id , $from ){
        $id = urldecode(base64_decode($user_id));
        $from = urldecode(base64_decode($from));
            // pre( $from );
            // die;
        if( $from == 'admin' ){
            $data = getQuery([ 'table' => 'admin_calendly_event' , 'select' => 'briefDesc' , 'where' => [ 'id' => $id ] , 'single' => true ]);
            $json_msg = $data;
            $json_msg["msg"] = "GET";
        }
        if( $from == 'counsellor' ){
            $data = getQuery([ 'table' => 'calendly_event' , 'select' => 'briefDesc' , 'where' => [ 'id' => $id ] , 'single' => true ]);
                // lQ(1);
            $json_msg = $data;
            $json_msg["msg"] = "GET";
        }
    ...
    public function email_send(){
            ...
            $this->email->from( $from , 'Respicite');
            $this->email->to($to);
    ...
    public function apply_skill_training( $training_id, $email , $action = 'CHECK_USER' ){
        // pre( [ $email  , $training_id ] , 1 );
        $json_msg = [];
        switch($action){
            case "CHECK_USER":
                    ...
                    if($q->num_rows() > 0){
                        ...
                        if( $userData['iam'] == 'user' ){
                            ...
                            if($this->User_model->assessment_otp_update_by_email($user_email,$OTP_code) > 0){
                                ...
                                $body_msg = email_template( [ 
                                    ...
                                    'heading2' => 'OTP For Verify in Respicite',
                                    'para1' => "Dear ".$userData['fullname']." <br/> <br/> Please complete your Otp Verification with Respicite
                                    by using the following OTP - <b>".$OTP_code."</b>"
                                 ] );
                                $this->User_model->otp_send_on_email($user_email,$subject,$body_msg);
                            }
                            $json_msg["msg"] = "NEW LOGIN";
                        ...
                        if($this->User_model->assessment_otp_update_by_email($email,$OTP_code) > 0){
                            ...
                            $body_msg = email_template( [ 
                                'heading' => 'Registration Information',
                                'heading2' => 'OTP For Verify in Respicite',
                                'para1' => "Dear ".$email." <br/> <br/> Please complete your Registration with Respicite
                                by using the following OTP - <b>".$OTP_code."</b><br/> Login By using of this password : - ".$password
                             ] );
                            $this->User_model->otp_send_on_email($email,$subject,$body_msg);
                        }
                        $json_msg["msg"] = "REGISTRATION";
