<?php 
    class AdminController extends CI_Controller
    {
        ...
        function StatusChangeByEmail($email=null)
        {
            if( $email != null  ){
                $getUser = getQuery( [ 'where' => [ 'email' => $email ], 'table' => 'user_details' , 'single' => true ]);
                if( !empty( $getUser ) ){
                    update( [ 'table' => 'user_details' , 'data' => [ 'status' => 1 , 'email_otp' => NULL ] ,  'where' => [ 'email' => $email ] ] );
                    if( affected() > 0 ){
                        echo "Email Status has been change , try login";
                    }
                    else{
                        echo "Email Status already has been change , try login";
                    }
                }
                else{
                    echo "Please Check Email does'nt exist , try agin with new";
                }
            }
            else{
                echo "Email Not Availabel";
            }
        }
        ...
        function variation_report_image($id=null){
            ...
            if( isset( $_GET['solution_name'] ) ){
                ...
                $data['getClass'] = getQuery( [ 'table' => ' solution_variant' ,  'where' => [ 'solution_name' => $_GET['solution_name'] ] , 'group_by' => [ 'variant_no' ]  ] );
            }
            // pre( $data , 1 );
            $data['mainmenu'] = $this->Commen_model->get_marketplace_menu($data['user']['iam'], 'dev');
            ...
            $this->load->view('admin/add_variation_image' , $data);
        ...
        function update_variation_image(){
            if( !empty( $_POST ) ){
                ...
                if(  $_FILES['pdf']['name'] != '' ){
                    ...
                    $dname = explode(".", $_FILES['pdf']['name']);

                    $ext = end($dname);

                    $new_name = $_POST['solution_name']."-variation-".$_POST['variationId']."-default.".$ext;
                    ...
                    if ( ! $this->upload->do_upload('pdf'))
                    {
                        ...
                        redirect('AdminController/variation-report-image');
                    }
                    else
                    {
                    ...
                    if( empty( $getData ) ){
                        ...
                        if( affected() > 0 ){
                            ...
                            redirect('AdminController/variation-image-list');
                        }
                        else{
        ...
        function variation_image_list(){
            ...
            $this->load->view('admin/variation-image-list' , $data);
        ...
        function service_list(){
            ...
            $data['list'] = getQuery( [ 'table' => 'retail_services' ] );
            ...
            $this->load->view('admin/retail-service-list' , $data);

            $this->load->view('footer');

        }

        function add_service($edit= null){
            ...
            if( $edit != null ){
                ...
                $data['edit'] = getQuery( [ 'table' => 'retail_services' , 'where' => [ 'id' => $edit ] , 'single'=> true ] );
            ...
            $this->load->view( 'admin/add-retail-service' , $data);
        ...
        function submit_service(){
            ...
            if( !empty( $_POST ) ){
                ...
                if(  $_FILES['image']['name'] != '' ){
                    ...
                    $new_name = str_replace(' ' , '-', $_POST['services']).'.'.$ext;
...

                    {
...

                        {
                            ...
                            redirect('AdminController/add-retail-service','refresh');
                ...
                if( !isset( $_POST['id'] ) ){
                    ...
                    $_POST['slug'] = strtolower(str_replace( ' ', '-' , $_POST['services'] ));

                    insert( 'retail_services' , $_POST );

                    if( affected() > 0 ){

                        $this->session->set_flashdata('msg','Services Added');

                        $this->session->set_flashdata('status','success');

                        redirect('AdminController/service-list','refresh');

                    }

                    else{

                        $this->session->set_flashdata('msg','Not Added, Try again');

                        $this->session->set_flashdata('status','warning');

                        redirect('AdminController/add-service','refresh');
                    ...
                    update( [ 'where' => [ 'id' => $_POST['id'] ] , 'table' => ' retail_services' , 'data' => $_POST ] );

                    if( affected() > 0 ){

                        $this->session->set_flashdata('msg','Services Updated');
                ...
                $this->session->set_flashdata('msg','Invalid Method');
        ...
        function service_detail_list($id = null){
            ...
            $data['list'] = getQuery( [ 'table' => 'retail_service_details', 'where' => [ 'service_id' => $id ] ] );
            ...
            $this->load->view('admin/retail_service_details' , $data );
        ...
        function service_status_update($id = null , $status = null){

            update([ 'where' => [ 'id' => $id ] , 'data' => [ 'status' =>$status ] , 'table' => 'retail_services' ]);
            ...
            if( affected() > 0 ){
                ...
                redirect( 'adminController/service-list' );

        }

        function counsellor_status_update( $id = null , $status = null ){

            update([ 'where' => [ 'id' => $id ] , 'data' => [ 'status' =>$status ] , 'table' => ' retail_counsellor_list' ]);
            ...
            if( affected() > 0 ){
                ...
                redirect( 'adminController/counsellor-list' );

        }

        function details_delete($id , $service_id){

            delete( [ 'table' => 'retail_service_details' , 'where' => [ 'id' => $id ] ] );

            if( affected() > 0){

                $this->session->set_flashdata( 'msg','Delete Service Details, successfully' );

                $this->session->set_flashdata( 'status','success' );

                redirect( 'adminController/service-detail-list/'.$service_id );
        ...
        function add_details($id = null , $action = null){
            ...
            if( $action == 'edit' ){
                ...
                $data['edit'] = getQuery( [ 'table' => 'retail_service_details' , 'where' => [ 'id' => $id ] , 'single'=> true ] );
            ...
            $this->load->view( 'admin/add-retail-details' , $data);
        ...
        function submit_service_details(){
            ...
            if( !empty( $_POST ) ){
                ...
                if( !isset( $_POST['id'] ) ){
                    ...
                    insert( ' retail_service_details' , $_POST );
                    ...
                    if( affected() > 0 ){
                        ...
                        redirect('AdminController/service-detail-list/'.$_POST['service_id'],'refresh');
                        ...
                        redirect('AdminController/add_details/'.$_POST['service_id']."/add",'refresh');
                    ...
                    update( [ 'where' => [ 'id' => $_POST['id'] ] , 'table' => ' retail_service_details' , 'data' => $_POST ] );
                    ...
                    if( affected() > 0 ){
                        ...
                        redirect('adminController/edit-details/'.$_POST['id']."/edit",'refresh');
        ...
        function counsellor_list(){
            ...
            $data['list'] = getQuery( [ 'table' => 'retail_counsellor_list' ] );
            ...
            $this->load->view('admin/counsellor_list' , $data);

            $this->load->view('footer');

        }

        function add_counsellor_details($id = null){
            ...
            if( $id != null ){
                ...
                $data['edit'] = getQuery( [ 'table' => 'retail_counsellor_list' , 'where' => [ 'id' => $id ] , 'single'=> true ] );
            ...
            $this->load->view( 'admin/add_counsellor_details' , $data);

            $this->load->view('footer');

        }

        function submit_counsellor_detail(){
            ...
            if( !empty( $_POST ) ){
                ...
                if(  $_FILES['image']['name'] != '' ){
                    ...
                    $new_name = ($_POST['first_name']."-".$_POST['last_name']).time().'.'.$ext;
                ...
                if( !isset( $_POST['id'] ) ){
                    ...
                    insert( ' retail_counsellor_list' , $_POST );

                    if( affected() > 0 ){

                        $this->session->set_flashdata('msg','Counsellor Added');

                        $this->session->set_flashdata('status','success');

                        redirect('AdminController/counsellor-list','refresh');
                        ...
                        redirect('AdminController/add-counsellor-details/','refresh');
                    ...
                    update( [ 'where' => [ 'id' => $_POST['id'] ] , 'table' => ' retail_counsellor_list' , 'data' => $_POST ] );

                    if( affected() > 0 ){

                        $this->session->set_flashdata('msg','Counsellor Updated');
                        ...
                        redirect('AdminController/edit-counsellor-details/'.$_POST['id']."/edit",'refresh');
        ...
        function counsellor_services($counsellorId = null){
            ...
            $data['selected'] = getQuery( [ 'table' => 'retail_service_counsellor_map' , 'where' => [ 'counsellor_id' => $counsellorId]] );
            ...
            $this->load->view('admin/counsellor_services_list' , $data );

            $this->load->view('footer');

        }

        function submit_counsellor_services(){

            if( !empty( $_POST ) ){

                // pre( array_keys($_POST['service_id']) , 1 );

                $list = getQuery( [ 'table' => 'retail_services' ] );

                foreach( $list as $value ){

                    foreach( $_POST['service_id'] as $k => $v  ){

                        $getService = getQuery([ 'where' => [ 'service_id' => $k , 'counsellor_id' => $_POST['counsellor_id'] ] , 'table' =>  'retail_service_counsellor_map' ]);

                        if( empty( $getService ) ){

                            if( $value['id'] == $k ){

                                    $data = [ 'service_id' => $k  , 'counsellor_id' => $_POST['counsellor_id'] ];

                                    insert( 'retail_service_counsellor_map' , $data );
                            ...
                            $arr_keys = array_keys($_POST['service_id']);

                            // pre( $arr_keys );

                            // echo in_array( $value['id'] , $arr_keys );

                            // echo $k." Key ";

                            if( !in_array( $value['id'] , $arr_keys ) ){

                                delete([ 'where' => [ 'service_id' => $value['id'] , 'counsellor_id' => $_POST['counsellor_id'] ] , 'table' =>  'retail_service_counsellor_map' ]);
                ...
                if( affected() > 0 ){
                    ...
                    $this->session->set_flashdata('msg','Counsellor Service Update');
                    ...
                    redirect('AdminController/counsellor-services/'.$_POST['counsellor_id']."/edit",'refresh');
        ...
        function review_status( $reviewId = null , $status = null ){

            if( $reviewId != null && $status != null ){

                update( [ 'where' => [ 'id' => $reviewId ] , 'table' => 'review_rating' , 'data' => [ 'status' => $status ] ] );
                ...
                if( affected() >  0){
                    ...
                    redirect('/AdminController/notifications','refresh');
        ...
        public function certification_update(){
            ...
            $solution_certification = [
                ...
                'Corporate Coaching' => [
                    'Corporate Coching' , 'WLA'
                ],
                'Overseas Services' => [
                    'Overseas Services' , 'OCS'
                ],
                'Parenting Counselling' => [
                    'Positive Parenting' , 'PPE'
                ],
            ];

            $insert = [];
            foreach ($users as $key => $value) {
                $getCertification = getQuery( [ 'where' => [ 'email' => $value['email'] ] , 'table' => 'reseller_certification' , 'single' => true ] );
                if( empty( $getCertification ) ){
                    if( $value['serviceName'] != 'Skill Development' || $value['serviceName']  != 'School'){
                        $insert[$key]['email'] = $value['email'];
                        $insert[$key]['c_group'] = $solution_certification[$value['serviceName']][0];
                        $insert[$key]['solution'] = $solution_certification[$value['serviceName']][1];
                        $insert[$key]['status'] = 2;
                        $insert[$key]['c_status'] = 1;
                        $insert[$key]['per'] = '83.33333333';
                        $insert[$key]['dt'] = date('m/d/Y');
                        $insert[$key]['rqst'] = 0;
                        $insert[$key]['completed_status'] = 'portal_completed';
                    }
                }
            }
            if( !empty($insert) ){
                $id=$this->db->insert_batch( 'reseller_certification' , $insert );
                if( $id  ){
                    $this->session->set_flashdata('msg','Reseller certification update');
                    $this->session->set_flashdata('status','success');
                    redirect( 'AdminController/view_reseller' );
                }
                else{
                    $this->session->set_flashdata('msg','Not updated try again');
                    $this->session->set_flashdata('status','info');
                ...
                $this->session->set_flashdata('msg','Certification already updated');
                $this->session->set_flashdata('status','info');
                redirect( 'AdminController/view_reseller' );
            }
        }
        ...
        public function calendly_config($id = null)

        {
            ...
            if( $id != null ){
                ...
                $data['edit'] = getQuery( [ 'table' => 'admin_calendly_event' , 'where' => [ 'id' => $id ]   , 'single' => true ] );
            ...
            $this->load->view('admin/book_link');
        ...
        public function calendly_link_list(){
            ...
            $this->load->view('admin/calendly_link_list');
        ...
        public function control_list(){
            ...
            $data['mainmenu'] = $this->Admin_model->get_marketplace_menu();

            $data['list'] = getQuery( [ 'table' => 'calendly_request_control', 'select' => 'calendly_request_control.* , user_details.fullname, user_details.iam'  , 'join' => [ [ 'user_details' , 'user_details.id = calendly_request_control.user_id' , 'INNER' ] ] , 'where' => [ 'calendly_request_control.status' => 'pending' ] ] );
            ...
            $this->load->view('admin/control_list');
        ...
        public function update_control( $id = null , $request_variable ){

            if( $id  != null ){

                $this->load->model('Admin_model');

                $data = getQuery( [ 'where' => [ 'id' => $id ]  , 'table' => 'calendly_request_control' , 'single' => true ] );

                update( [ 'where' => [ 'id' => $id ] , 'data' => [ 'status' => 'success' ], 'table' => 'calendly_request_control' ] );

                $userData = $this->Admin_model->getUserDetailsById($data['user_id']);
                ...
                if (in_array($services, $rr)){
                    ...
                    delete( [ 'table' => 'calendly_event' , 'where' => [ 'user_id' => $data['user_id'] ] ] );

                    $this->Admin_model->allowed_services_update($data['user_id'],$allow);

                }

                else{

                    $this->Admin_model->allowed_services_update( $data['user_id'], $userData.','.$services);
                ...
                if( affected()  >  0){
                    ...
                    $this->session->set_flashdata("msg","Updated data successfully");

                    $this->session->set_flashdata("status","success");

                    redirect('/AdminController/control-list');
...

        {
            ...
            $this->form_validation->set_rules('event_name','Event Book','required');

            $this->form_validation->set_rules('briefDesc','Brief Description','required');

            $this->form_validation->set_rules('desc','Decription','required');
...

            {
                ...
                if( isset( $_POST['id'] ) ){
                    ...
                    $id  =  update([ 'table' => 'admin_calendly_event' , 'data' => $_POST , 'where' => [ 'id' => $_POST['id'] ] ] );

                    if( affected() > 0 ){

                        $this->session->set_flashdata("msg","Update successfully");

                        redirect('/AdminController/calendly-link-list');    

                    }

                    else{

                        $this->session->set_flashdata("msg","Not Updated successfull , Try again");

                        redirect('/AdminController/edit-calendly-config/'.$_POST['id']);
                    ...
                    $id  =  insert( 'admin_calendly_event' , $_POST );
                    ...
                    if( affected() > 0 ){
                        ...
                        $this->session->set_flashdata("msg","Not added successfull , Try again");

                        redirect('/AdminController/calendly-config');
...

            {
                ...
                if( isset( $_POST['id'] ) ){
                    ...
                    redirect('/UserController/edit_booking_link/'.$_POST['id'],'refresh');
                    ...
                    redirect('/UserController/add_booking_link','refresh');
        ...
        public function rating_status( $reseller_id = null , $status = null ){

            if( $reseller_id != null  && $status != null ){

                update( [ 'where' => [ 'id' => $reseller_id ] , 'data' => [ 'rating_status' => $status ] , 'table' => 'user_details' ] );

                // lQ(1);

                if( affected() > 0 ){

                    $this->session->set_flashdata('msg','Rating status Updated');
                    ...
                    $this->session->set_flashdata('msg','Status Remians Same');
            ...
            redirect('AdminController/confg_counseling_link');

        }

        public function rating_nature( $reseller_id = null , $status = null ){

            if( $reseller_id != null  && $status != null ){

                update( [ 'where' => [ 'id' => $reseller_id ] , 'data' => [ 'rating_nature' => $status ] , 'table' => 'user_details' ] );
...

        {
            ...
            $data['ap_book_data'] = getQuery([ 'table' =>'user_book_appointment', 'select' => 'user_book_appointment.* , reseller.fullname as resellerName',

                    'order' => [ 'user_book_appointment.id' => 'DESC'], 

                    'join' => [     

                    [

                        'user_details as reseller' , 

                        'reseller.id = user_book_appointment.user_id' , 
            ...
            // pre( $data['ap_book_data'] , 1 );

            $this->load->model('Admin_model');

            // $data['spResellers'] = $this->User_model->spResellers();
...

        {
            ...
            $getDomain = getQuery([ 'where' => [ 'id' => $id ] , 'table' => 'reseller_homepage' ,'single' => true ]);
            ...
            if( !empty( $getDomain ) ){
                ...
                $this->db->where( 'email' , $getDomain['r_email'] );

                $this->db->set( 'user_id' , $getDomain['reseller_id'] );
        ...
        function seo_parameter($id=null){     
            ...
            $data['user'] = $user;
            if( $id != null ){
                $data['edit'] = getQuery([ 'where' => [ 'id' => $id ] , 'table' => 'seo_faq' , 'single' => true ]);
            }

            $data['faq'] = getQuery([ 'table' =>  'seo_faq' ]);
        ...
        function seo_page_delete($id = null)
        {
            if( $id != null ){
                delete( [ 'where' => [ 'id' => $id ] , 'table' => 'seo_faq' ] );
                if( affected() > 0 ){
                    $this->session->set_flashdata('msg','Deleted Question');
        ...
        function page_seo_update()
        {
            if( !empty( $_POST ) ){
                if( isset( $_POST['id'] ) ){
                    update( [ 'where' => [ 'id' => $_POST['id'] ] , 'data' => $_POST , 'table' => 'seo_faq' ] );
                    if( affected() > 0 ){
                        $this->session->set_flashdata('msg','Updated Question');
                        redirect(base_url().'/AdminController/seo_parameter');
                    }
                    else{
                        $this->session->set_flashdata('msg','Updated Remain\'s Same');
                        redirect(base_url().'/AdminController/seo_parameter');
                    }
                }
                else{
                    insert( 'seo_faq' , $_POST );
                    // lQ(1);
                    if( affected() > 0 ){
                        $this->session->set_flashdata('msg','Added Question');
                        redirect(base_url().'/AdminController/seo_parameter');
                    }
                    else{
                        $this->session->set_flashdata('msg','Please Try Again');
        ...
        public function create_user_link(){
            $users = getQuery( [ 'where_in' => [ 'iam' => ['sp' , 'reseller']] ,'where' => [ 'profile_link' => '' ] , 'table' => 'user_details' ] );
            // pre(  count($users) , 1);
            if( !empty( $users ) ){

                foreach( $users as $key => $user ){
                    $data['profile_link'] = getProfileLink( $user['fullname'] );
                ...
                $this->session->set_flashdata('msg','No New SP/Reseller Founded');
                redirect(base_url().'AdminController/view_reseller');
        ...
        public function image_sync(){
            $getAllUsersImage = getQuery(  [ 'select' => 'id, profile_link , profile_photo' ,  'where_in' => [ 'iam' => [  'sp' , 'rese' ] ] , 'table' => 'user_details' , 'where' => [ 'profile_photo !=' => 'uploads/default.png'  ] ] );
            $notOriginalExist = [];
            // pre( $notOriginalExist , 1 );
            foreach ($getAllUsersImage as $key => $value) {
                if( file_exists("./".$value['profile_photo']) ){
                    $photoName = explode('/' , $value['profile_photo']);
                    if( !file_exists( "./uploads/original/".$photoName[1] ) ){
                        $value['photoName'] = $photoName[1];
                        $value['photoName'] = $photoName[1];
                        $notOriginalExist[] = $value;
                    }
                }
            }
            // pre( $notOriginalExist , 1 );
            if( !empty( $notOriginalExist ) ){
                foreach ($notOriginalExist as $key => $value){
                    $new_name = $value['profile_link'].'.jpg';
                    update([ 'table' => 'user_details' , 'data' => [ 'profile_photo' => 'uploads/'.$new_name ] , 'where' => [ 'id' => $value['id'] ] ]);
                    rename("./".$value['profile_photo'], './uploads/original/'.$new_name );
                ...
                // echo 'hello';
                $this->session->set_flashdata('msg','No New Image Founded');
                redirect(base_url().'AdminController/view_reseller');
            }
        }
        ...
        // Delete list_update_old_5_9_2024 after verification
        public function list_update_old_5_9_2024(){
