              ...
              <div class="card ">
                ...
                <?php
                    ...
                    if(isset( $_GET['solution_name' ])){
                      $url = base_url().'adminController/update_variation_image';
                      $method = 'post';
                    }
                    else{
                      $url = '';
                ...
                <form action="<?= $url ?>" method="<?= $method ?>" enctype='multipart/form-data'>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="solution_name" required autocomplete="off">
                        ...
                        ?>
                            <div class="form-group mt-4">
                              <label for="cluster_path">Select Variantion Number</label>
                              <select class="form-control" name="variationId" required autocomplete="off">
                                <option value=''>Select Variantion</option>
                                <?php
                                    if( !empty($getClass) ){
                                        foreach( $getClass as $key => $value ){
                                ?>
                                        <option value='<?= $value['variant_no'] ?>' <?= isset($edit) ? ( $edit['variant_no'] == $value['variant_no'] ? 'selected' : '' ) : '' ?> > 
                                        <?= $value['variant_no'] ?>  
                                        
                                         <!-- $value['value_par_1'] == 'na' ? ucwords(str_replace('_' , ' ' , $value['variant_name'])) : ucwords(str_replace('_' , ' ' , $value['value_par_1']))  -->
                                          <!-- $value['par_2'] == 'stream_change' && $value['value_par_2'] == 'yes' ? 'With Stream Change' :   -->
                                      </option>
                                <?php
                                        }                         
                                    }
                            ...
                            <div class="form-group mt-4">
                              <label for="career_path">Upload Pdf</label>
