            ...
            <div class="card ">
                ...
                ?>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="careerPath"   name="Path">
                            ...
                            <?php
                                ...
                                ?>
                                        <option value='<?= $value['Path'] ?>' <?= isset($edit) ? ( $edit['Path'] == $value['Path'] ? 'selected' : '' ) : '' ?> > <?= ucwords($value['Path']) ?> </option>
                                <?php
                                        }
                                    }
                                }
                    ...
                    <?php
                        // pre( $checkProfession );
                        if( isset( $edit ) ){
                            if( empty( $checkProfession )  ){
                                if( !empty( $professionList ) ){
                    ?>
                        <div class="form-group">
                            <label for="career_path">Select Profession Name ( Optional )</label>
                            <select  class='form-control select2' name='c_lib_1_profession_id'>
                                <option> Select Profession Name </option>
                        <?php
                                    foreach( $professionList as $k => $v ){
                        ?>
                                        <option value='<?= $v['SNo'] ?>' <?= !empty( $checkProfessionClib ) ? ( $checkProfessionClib['profession_name'] == $v['profession_name'] ? 'selected' : '' ) : '' ?> > <?= $v['profession_name'] ?> </option>
                        <?php
                                    }
                        ?>
                            </select>
                        ...
                        <div class='form-group'>
                            <label for='cdb_status' >cdb status</label>
                            <select class='form-control' name='cdb_status'>
                                <option> Select Status </option>
                                <option value='added' <?= isset($edit) ? ($edit['cdb_status'] == 'added'  ? 'selected' : '') : '' ?> >Added</option>
                                <option value='to_add'  <?= isset($edit) ? ($edit['cdb_status'] == 'to_add'  ? 'selected' : '') : '' ?> >To add</option>
                            </select>
                        </div>
                    <?php
                                }
