              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-bordered">
                  ...
                  <?php 
                              ...
                              {
                                  $rating = getQuery( [ 'select' => 'count(id) as id, SUM(rating) as total_rating', 'where' => [ 'reseller_id' => $row2['id'] ] , 'table' => 'review_rating' , 'single' => true ]);
                                //   lQ(1) ;
                                echo "<tr>";
                                ?>
                                ...
                                <td>
                                    <a class='btn btn-<?= $row2['rating_nature'] == 'manual' ? 'warning' : 'success' ?>' href='<?= base_url() ?>adminController/rating-nature/<?= $row2['id'] ?>/<?= $row2['rating_nature'] == 'manual' ? 'automated' : 'manual' ?>' ><?= ucwords( $row2['rating_nature'] ) ?></a>
                                    <?php
                                        if( $row2['rating_nature'] == 'automated' ){
                                    ?>
                                            <button class='btn mt-1 mb-1 btn-warning' ><?= $row2['rating_value_automated'] ?> </button>    
                                    <?php
                                        }
                                        else{
                                    ?>
                                        <button class='btn mt-1 mb-1 btn-warning' ><?= $row2['rating_value_manual'] ?></button>
                                    <?php   
                                        }
                                    ?>
                                    <a class='btn btn-<?= $row2['rating_status'] == 'active' ? 'success' : 'warning' ?>' href='<?= base_url() ?>adminController/rating-status/<?= $row2['id'] ?>/<?= $row2['rating_status'] == 'active' ? 'inactive' : 'active' ?>' ><?= $row2['rating_status'] == 'active' ? 'active' : 'inactive' ?></button>
                                </td>
                                <td>
